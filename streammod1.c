...
#pragma omp parallel for
    ...
    /* Open file for writing timestamps */
    FILE *fp = fopen("stream_timestamps.txt", "w");
    if (fp == NULL) {
...
#pragma omp parallel for
    ...
    printf("Data saved to: stream_timestamps.txt\n");
...
void print_timestamp(double timestamp, FILE *fp)
{
    ...
    // Convert timestamp back to timeval
    tp.tv_sec = (time_t)timestamp;
    tp.tv_usec = (suseconds_t)((timestamp - tp.tv_sec) * 1000000);
    ...
    printf("%s.%06ld", buffer, tp.tv_usec);
    if (fp != NULL) {
        fprintf(fp, "%s.%06ld", buffer, tp.tv_usec);
    }
}
